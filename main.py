...
async def trigger_bitrise_build(
    ...
    pipeline_id: str = Field(
        default=None,
        description="The pipeline name to build",
    ),
    ...
    commit_hash: str = Field(
        default=None,
        description="The commit hash for the build (optional)",
    ),
) -> str:
    url = f"{BITRISE_API_BASE}/apps/{app_slug}/builds"
    ...
    if pipeline_id:
        build_params["pipeline_id"] = workflow_id
    if commit_message:
...
@mcp_tool(
    api_groups=["builds"],
    description="Trigger a new simplified build/pipeline for a specified Bitrise app.",
)
async def trigger_bitrise_pipeline_build(
    app_slug: str = Field(
        description='Identifier of the Bitrise app (e.g., "d8db74e2675d54c4" or "8eb495d0-f653-4eed-910b-8d6b56cc0ec7")',
    ),
    branch: str = Field(
